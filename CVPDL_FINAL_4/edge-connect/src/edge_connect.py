    ...
    def test(self):
        ...
        for items in test_loader:
            ...
            # inpaint with edge model / joint model
                ...
                # 對 edge model 的輸出進行尺寸調整以匹配 images
                edges = F.interpolate(edges, size=(images.size(2), images.size(3)), mode='bilinear', align_corners=False)

                outputs = self.inpaint_model(images, edges, masks)

                # 對 inpaint model 的輸出進行尺寸調整以匹配 images
                outputs = F.interpolate(outputs, size=(images.size(2), images.size(3)), mode='bilinear', align_corners=False)
